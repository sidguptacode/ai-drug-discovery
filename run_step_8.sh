#!/usr/bin/env bash
# Run pipeline step 8 (LR scoring). Uses config.yml unless PIPELINE_STEP_CONFIG is set. Execute from repo root.
...
LOG_FILE=""
[[ -n "$RUN_ID" ]] && LOG_DIR="$REPO_ROOT/runs/$RUN_ID/logs" && mkdir -p "$LOG_DIR" && LOG_FILE="$LOG_DIR/step_8.log"
...
STEP_SCRIPT="separated_steps/step_8.py"
[[ -n "$RUN_ID" ]] && [[ -f "$REPO_ROOT/runs/$RUN_ID/steps/step_8.py" ]] && STEP_SCRIPT="$REPO_ROOT/runs/$RUN_ID/steps/step_8.py"

echo "====== Step 8 — LR scoring ======"
if [[ -n "$LOG_FILE" ]]; then
