#!/usr/bin/env bash
# Run pipeline step 5 (Export). Uses config.yml unless PIPELINE_STEP_CONFIG is set. Execute from repo root.
...
LOG_FILE=""
[[ -n "$RUN_ID" ]] && LOG_DIR="$REPO_ROOT/runs/$RUN_ID/logs" && mkdir -p "$LOG_DIR" && LOG_FILE="$LOG_DIR/step_5.log"

# Run-specific script override: separated_steps/ is static; use runs/<run_id>/steps/ when present
STEP_SCRIPT="separated_steps/step_5.R"
[[ -n "$RUN_ID" ]] && [[ -f "$REPO_ROOT/runs/$RUN_ID/steps/step_5.R" ]] && STEP_SCRIPT="$REPO_ROOT/runs/$RUN_ID/steps/step_5.R"
