#!/usr/bin/env bash
# Run pipeline step 3 (Clustering). Uses config.yml unless PIPELINE_STEP_CONFIG is set. Execute from repo root.
...
LOG_FILE=""
[[ -n "$RUN_ID" ]] && LOG_DIR="$REPO_ROOT/runs/$RUN_ID/logs" && mkdir -p "$LOG_DIR" && LOG_FILE="$LOG_DIR/step_3.log"

# Run-specific script override: separated_steps/ is static; use runs/<run_id>/steps/ when present
STEP_SCRIPT="separated_steps/step_3.R"
[[ -n "$RUN_ID" ]] && [[ -f "$REPO_ROOT/runs/$RUN_ID/steps/step_3.R" ]] && STEP_SCRIPT="$REPO_ROOT/runs/$RUN_ID/steps/step_3.R"
