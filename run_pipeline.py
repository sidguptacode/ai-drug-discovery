...
Tools (for agent or programmatic use):
  ...
  - run_step(step, run_dir, project_root=None) -> int   # 0 = success, else exit code
  - setup_run_dir_from_config(config_path, run_dir, custom_config_path=None) -> None

CLI: interactive loop with two options at each step: [r]erun | [n]ext.
Assumes step JSONs (step_1.json ... step_10.json) exist in run_dir; you or the agent write them.
Use --init to create run_dir and seed JSONs from config.yml once.
"""
...
def setup_run_dir_from_config(
    ...
    After this, agent or human can edit step_*.json before/between run_step calls.
    """
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        required=True,
        help="Run directory containing step_1.json ... step_10.json (create with --init)",
    )
    parser.add_argument(
        "--init",
        action="store_true",
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Start from last completed step (read run_dir/last_step.txt); only with interactive loop",
    )
    ...
    if args.init:
        ...
        if not config_path.exists():
            ...
            sys.exit(1)
        setup_run_dir_from_config(str(config_path), run_dir, custom)

    if not run_dir.is_dir():
        print(f"ERROR: Run dir not found: {run_dir}. Use --init to create from config.", file=sys.stderr)
        sys.exit(1)

    # Single-step mode (agent can call: run_pipeline.py --run-dir X --step 3)
