#!/usr/bin/env bash
# Run pipeline step 7 (Preprocess). Uses config.yml unless PIPELINE_STEP_CONFIG is set. Execute from repo root.
...
LOG_FILE=""
[[ -n "$RUN_ID" ]] && LOG_DIR="$REPO_ROOT/runs/$RUN_ID/logs" && mkdir -p "$LOG_DIR" && LOG_FILE="$LOG_DIR/step_7.log"
...
STEP_SCRIPT="separated_steps/step_7.py"
[[ -n "$RUN_ID" ]] && [[ -f "$REPO_ROOT/runs/$RUN_ID/steps/step_7.py" ]] && STEP_SCRIPT="$REPO_ROOT/runs/$RUN_ID/steps/step_7.py"

echo "====== Step 7 — Preprocess ======"
if [[ -n "$LOG_FILE" ]]; then
